    ...
    return (
        <Box>
            ...
            <Dialog open={open} onClose={handleClose}>
                <DialogTitle sx={{backgroundColor: '#202E4B', color: 'white'}}>Record Your Grocery Expenditures</DialogTitle>
                ...
                <DialogContent sx={dialogLeft}>
                    ...
                    <TextField
                        ...
                        type={'date'}
                        value={groceryDate}
                        onChange={(event) => { setGroceryDate(event.target.value) }}
                    />
                    <TextField
                        autoFocus
                        margin="dense"
                        ...
                        variant="standard"
                        value={groceryStore}
                        onChange={(event) => { setGroceryStore(event.target.value) }}
                    />
                    </Box>
                    <FormControlLabel control={<Checkbox  checked={sale}
                           onChange={handleCheckboxChange}
                    />}  label={'Sale'}  labelPlacement="start" sx={{justifyContent: 'left', margin: 0, userSelect: 'none'}}/>
                    <Button variant='contained' sx={buttonStyling} onClick={handleAddItem}
                    disabled={groceryName == '' || groceryPrice == '' || groceryDate == '' || groceryStore == '' && true}>Add Item</Button>
                </DialogContent>
                ...
                <DialogActions>
                    ...
                    <Button variant={'contained'} sx={buttonStyling}onClick={handleSubmit}>Submit</Button>
                </DialogActions>
            </Dialog>
        </Box>
    );
